...
const GuestList = () => {
  ...
  return (
    <section className="lg:px-16 sm:px-8 px-3 py-10 font-raleway">
      ...
      <p className=" text-graywhite-400 font-lato lg:text-2xl my-2">My Guest List</p>
      ...
      <p>Guest Name</p>
        ...
        {guests.map((guest) => (
          <div key={guest.id} className="max-sm:text-xs text-graywhite-400 font-lato p-5 border grid grid-cols-6 gap-2">
            <p>{guest.guestName}</p>
            <p>{guest.group}</p>
            <p className=" break-words">{guest.email}</p>
            <p><div className="p-4 border rounded-full w-6 h-6 mx-auto"></div></p>
            <p>{guest.status}</p>
            <p className="flex flex-wrap gap-1 justify-center">
          <button className="cursor-pointer text-burgundy-100 border border-burgundy-200 sm:px-2 rounded">View</button>
          <button onClick={() => handleDelete(guest.id)}  className="cursor-pointer text-red-500 border border-red-500 sm:px-2 rounded">Delete</button>
          </p>
          </div>
        ))}
      ...
      <AddGuestlist isOpen={isSidebarOpen} closeSidebar={() => setIsSidebarOpen(false)} addNewGuest={addNewGuest} />
